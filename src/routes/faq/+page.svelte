<script lang="ts">
	...
	const faqs: Array<{ q: string; a: Answer }> = [
		...
		{
			q: 'In Configure for Auction, what are the rules profiles (Common PEI, Wikipedia, Rec Hall PEI, Tignish PEI)?',
			a: 'Each profile is a set of configuration values for a style of play. Common PEI is the default and matches how the game is most often played on PEI. Wikipedia follows the Wikipedia baseline. Rec Hall PEI is a faster kitty-less game. Tignish PEI is like Common PEI but deals a five-card kitty. You can also choose Custom and set each value yourself.'
		},
		{
